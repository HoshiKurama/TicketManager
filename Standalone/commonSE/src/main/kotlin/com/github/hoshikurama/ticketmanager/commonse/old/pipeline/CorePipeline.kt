...
import com.github.hoshikurama.ticketmanager.commonse.api.impl.CreatorSTD
import com.github.hoshikurama.ticketmanager.commonse.api.impl.TicketSTD
import com.github.hoshikurama.ticketmanager.commonse.api.impl.toLocaledWord
...
import com.github.hoshikurama.ticketmanager.commonse.old.misc.kyoriComponentDSL.buildComponent
import com.github.hoshikurama.ticketmanager.commonse.old.misc.kyoriComponentDSL.onHover
...
typealias ConsoleObject = CreatorSTD.ConsoleImpl
typealias ConsoleSender = Console
typealias ResultDB = DBResult
...
class CorePipeline(
    ...
    private fun hasValidPermission(
        ...
        fun hasDuality(basePerm: String): Boolean {
            val ownsTicket = ticket.creator == CreatorSTD.UserSTD(player.uniqueID)
    ...
    private fun allAssignVariations(
        ...
        TMCoroutine.runAsync {
            instanceState.database.insertAction(
                id = ticket.id,
                action = TicketSTD.ActionSTD(TicketSTD.ActionSTD.AssignSTD(dbAssignment), sender.toCreator(), sender.getLocAsTicketLoc())
            )
        }
    ...
    private fun closeWithComment(
        ...
        instanceState.database.run {
            TMCoroutine.runAsync {
                insertAction(
                    id = ticket.id,
                    action = TicketSTD.ActionSTD(TicketSTD.ActionSTD.CommentSTD(message), sender.toCreator(), sender.getLocAsTicketLoc())
                )
                insertAction(
                    id = ticket.id,
                    action = TicketSTD.ActionSTD(TicketSTD.ActionSTD.CloseSTD, sender.toCreator(), sender.getLocAsTicketLoc())
                )
    ...
    private suspend fun create(
        ...
        val ticket = when (sender) {
            is OnlinePlayer -> TicketSTD(creator = CreatorSTD.UserSTD(sender.uniqueID))
            is ConsoleSender -> TicketSTD(creator = ConsoleObject)
        }
            .let { it + listOf(TicketSTD.ActionSTD(type = TicketSTD.ActionSTD.OpenSTD(message), user = sender.toCreator(), sender.getLocAsTicketLoc())) }


        // Inserts ticket and receives ID
        val id = instanceState.database.insertNewTicketAsync(ticket)
    ...
    private suspend fun history(
        ...
        val searchedUser: Creator = targetName?.run { platform.offlinePlayerNameToUUIDOrNull(this)?.let(CreatorSTD::UserSTD)
            ?: CreatorSTD.UserSTD(UUID.randomUUID()) } ?: ConsoleObject //NOTE: Does this need to account for multi-servers?
    ...
    private fun reopen(
        ...
        val action = TicketSTD.ActionSTD(TicketSTD.ActionSTD.ReopenSTD, sender.toCreator(), sender.getLocAsTicketLoc())
    ...
    private suspend fun search(
        ...
        fun attemptNameToCreator(name: String): Option<Creator> {
            ...
            else platform.offlinePlayerNameToUUIDOrNull(name)?.let { Option(CreatorSTD.UserSTD(it)) } ?: Option(
                CreatorSTD.InvalidUUID)
        }
    ...
    private fun setPriority(
        ...
        TMCoroutine.runAsync {
            instanceState.database.insertAction(
                id = ticket.id,
                action = TicketSTD.ActionSTD(TicketSTD.ActionSTD.SetPrioritySTD(byteToPriority(args[2].toByteOrNull() ?: 3)), sender.toCreator(), sender.getLocAsTicketLoc())
            )
            ...
            .map {
                sender.locale.viewComment.parseMiniMessage(
                    ...
                    "comment" templated when (it.type) {
                        is TicketSTD.ActionSTD.CommentSTD -> (it.type as TicketSTD.ActionSTD.CommentSTD).comment
    ...
    private fun viewDeep(
        ...
        fun formatDeepAction(action: TicketSTD.ActionSTD): Component {
...
fun Sender.toCreator(): Creator {
    return when(this) {
        is OnlinePlayer -> CreatorSTD.UserSTD(uniqueID)
