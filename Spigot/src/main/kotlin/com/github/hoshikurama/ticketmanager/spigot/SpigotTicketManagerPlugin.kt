...
class SpigotTicketManagerPlugin(
    mainPlugin: SpigotPlugin,
    mainDispatcher: CoroutineDispatcher,
    asyncDispatcher: CoroutineDispatcher,
    private val adventure: BukkitAudiences,
) : TicketManagerPlugin<SpigotPlugin>(
    mainPlugin, mainDispatcher, asyncDispatcher
) {
    ...
    override suspend fun performRegistration() {
        ...
        configState.localeHandler.getCommandBases().forEach {
            mainPlugin.getCommand(it)!!.setSuspendingExecutor(commandPipeline as SpigotCommandPipeline)
            mainPlugin.getCommand(it)!!.tabCompleter = SpigotTabComplete(this, mainPlugin.perms, adventure)
            // Remember to register any keyword in plugin.yml
        }
        // Registers player join event
        mainPlugin.server.pluginManager.registerSuspendingEvents(SpigotPlayerJoinEvent(this, mainPlugin.perms, adventure), mainPlugin)
    }
}