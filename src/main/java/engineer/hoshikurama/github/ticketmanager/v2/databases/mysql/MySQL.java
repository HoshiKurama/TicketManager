package engineer.hoshikurama.github.ticketmanager.v2.databases.mysql;
...
import static engineer.hoshikurama.github.ticketmanager.v2.TMCommands.*;

public class MySQL implements Database {
    private final HikariDataSource ds;

    public MySQL(final String host, final String port, final String dbname, final String username, final String password) throws DatabaseException {
        try {
            ...
            // Tests connection
            Connection test = ds.getConnection();
            test.close();
    ...
    private boolean tableDoesNotExist(Connection conn) throws SQLException {
        boolean tExists = false;
        try (ResultSet rs = conn.getMetaData().getTables(null, null, "TicketManagerTicketsV2", null)) {
            while (rs.next()) {
    ...
    public void convertToSQLite(SQLite sqLite) throws DatabaseException {
        try (Connection connection = ds.getConnection()) {
            //Wipe destination
            sqLite.wipeTable();
            ...
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                sqLite.createTicket(parseTicket(rs.getInt(1), rs.getString(2), rs.getByte(3), rs.getString(4),
                        rs.getString(5), rs.getString(6), rs.getString(7), rs.getLong(8), rs.getString(9),
                        rs.getBoolean(10)));
            }
        } catch (SQLException e) {
    ...
    public void wipeTable() throws DatabaseException {
        try (Connection connection = ds.getConnection()) {
            connection.prepareStatement("TRUNCATE TABLE TicketManagerTicketsV2").execute();
        } catch (SQLException e) {
            throw new DatabaseException(e.getMessage(), e.getCause());
        }
    }
