...
import com.github.hoshikurama.ticketmanager.misc.mapToCreatorOrNull
...
class MySQL(
    ...
    override fun getOpenTicketsAsync(page: Int, pageSize: Int): CompletableFuture<Result> {
        return ticketsFilteredByAsync(page, pageSize, "SELECT ID FROM TicketManager_V8_Tickets WHERE STATUS = ?;", listOf(Ticket.Status.OPEN.name))
    }
    ...
    override fun getOpenTicketsNotAssignedAsync(page: Int, pageSize: Int): CompletableFuture<Result> {
        return ticketsFilteredByAsync(page, pageSize, "SELECT ID FROM TicketManager_V8_Tickets WHERE STATUS = ? AND ASSIGNED_TO IS NULL", listOf(Ticket.Status.OPEN.name))
    }
    ...
    override fun countOpenTicketsAsync(): CompletableFuture<Long> {
        return connectionPool.sendPreparedStatement("SELECT COUNT(*) FROM TicketManager_V8_Tickets WHERE STATUS = ?;", listOf(Ticket.Status.OPEN.name))
